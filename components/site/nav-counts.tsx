...
export function NavCount({ kind }: { kind: "bag" | "wishlist" }) {
  ...
  return (
    <span
      ...
      className="absolute top-0 right-0 flex h-[18px] min-w-[18px] items-center justify-center rounded-full bg-brand-red px-1 text-[10px] leading-none font-bold text-white tabular-nums"
    >
      {n > 99 ? "99+" : n}
    </span>
  );
